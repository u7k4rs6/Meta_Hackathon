...
def grade_task2(action, target_line):
    if action.line_number != target_line:
        return 0.1, f"Fix must be applied to line {target_line}."
    
    if not action.fix_code:
        return 0.1, "Fix code is empty."
    ...
    try:
        ...
        return 0.6, "Syntactically valid Python, but not the correct fix. (Partial Success)"
    except SyntaxError:
